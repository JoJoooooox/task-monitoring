...
<body>
	...
	<div class="main-wrapper" id="main-div">
        ...
        @include('observer.body.sidebar')

        <div class="page-wrapper">

            <!-- partial:partials/_navbar.html -->
            @include('observer.body.header')
            <!-- partial -->

            @yield('observer')

            <!-- partial:partials/_footer.html -->
            @include('observer.body.footer')
            <!-- partial -->
	...
	<script src="{{ asset('assets/custom-js/observer.js') }}"></script>
	<script src="{{ asset('assets/vendors/chartjs/Chart.min.js') }}"></script>
	...
	$(document).ready(function () {
        ...
        function forceLogout() {
            window.location.href = "/manager/logout"; // Update with your logout route
        }
		...
		function checkUrlUser() {
			...
			const allowedUrl = 'https://tribo.uno/manager/chat';

			if (currentUrl !== allowedUrl) {
				$.ajax({
					url: "{{ route('observer.chat.unsetchatishere') }}",
					type: "POST",
					noLoading: true,
					headers: {
				...
				$.ajax({
					url: "{{ route('observer.chat.setchatishere') }}",
					type: "POST",
					noLoading: true,
					headers: {
                        'X-CSRF-TOKEN': tokenDashboard
		...
		const profileUrl = 'https://tribo.uno/manager/profile';
		...
		function getAllFeedback(){
			...
			if(currentUrl !== profileUrl){
				$.ajax({
					url: `{{ route('observer.getallfeedback') }}`,
					type: 'GET',
					dataType: 'json',
					success: function(response) {
						if(response.status === 'exist' && response.feed.length > 0){
							var feedback_html = `
							<button class="btn dashboard-feedback-btn show"><i class="mdi mdi-thumb-up icon-wiggle" style="font-size: 19px;"></i></button>
							<div class="dashboard-feedback-container">
								<div class="input-group dashboard-feedback-search">
									<input type="text" class="form-control" id="searchAllFeedback" placeholder="Search here...">
								</div>
								<button class="btn dashboard-feedback-container-btn"><i class="mdi mdi-close icon-wiggle" style="font-size: 10px;"></i></button>
								<div class="dashboard-feedback-items">
							`;

							if(response.feed.length > 0){
								response.feed.forEach(feedback => {
									feedback_html += `
									<div class="dashboard-feedback-content" id="feed-div-${feedback.id}" data-name="${feedback.from_name}">
										${feedback.status == 'Pending' ? `<h5 class="dashboard-feedback-indicator" id="feedIndicator_${feedback.id}"><span class="badge text-bg-danger">New!</span></h5>` : ''}
										<div class="dashboard-feedback-header text-center">
											<p>Feedback by: <b class="text-primary">${feedback.from_name}</b></p>
										</div>
										<div class="dashboard-feedback-body">
											<p class="text-dark"><i data-feather="maximize" class="icon-lg icon-wiggle open-feedback" data-feed="${feedback.id}" type="button"></i></p>
											<p class="text-dark ms-2"><i data-feather="trash" class="icon-lg icon-wiggle delete-feedback" type="button" data-feed="${feedback.id}"></i></p>
										</div>
									</div>
									`;
								});
							...
							if(response.feed.length > 0){
								response.feed.forEach(feedback => {
									feedback_html += `
									<div class="dashboard-feedback-info" id="feedback-info-${feedback.id}">
										<button class="btn dashboard-feedback-info-btn"><i class="mdi mdi-subdirectory-arrow-left icon-wiggle" style="font-size: 15px;"></i></button>
										<div class="row">
											<div class="col-12 mb-2 text-center d-flex justify-content-center align-items-center">
												<p style="width: 260px; padding-inline: 20px !important; padding: 10px !important; font-size: 15px; box-shadow: 4px 4px 0 0 rgba(101, 113, 255, .25) !important; color: #343434; border: 1px solid #B4B2B2; border-radius: 20px; background: rgba(231, 231, 231, 0.75);">Feedback by: <b>${feedback.from_name}</b></p>
											</div>
											<div class="col-12 mb-2 text-center d-flex justify-content-center align-items-center">
												<p class="d-flex justify-content-center align-items-center me-2" style="width: 40px; padding: 10px !important; font-size: 13px; box-shadow: 4px 4px 0 0 rgba(101, 113, 255, 0.25); color: #343434; border: 1px solid #B4B2B2; border-radius: 10px; background: rgba(231, 231, 231, 0.75);"><i class="mdi mdi-star icon-wiggle text-warning"></i>${feedback.rate}</p>

												<i class="mdi mdi-color-helper" style="font-size: 13px; color: rgba(231, 231, 231, 1); text-shadow: 2px 2px 0 rgba(101, 113, 255, 0.25); text-shadow: -1px -1px 0 #B4B2B2, 0px -1px 0 #B4B2B2, 1px -1px 0 #B4B2B2, -1px  0px 0 #B4B2B2, 1px  0px 0 #B4B2B2, -1px  1px 0 #B4B2B2, 0px  1px 0 #B4B2B2, 1px  1px 0 #B4B2B2;"></i>

												<p class="ms-2" style="width: auto; padding: 10px !important; font-size: 13px; box-shadow: 4px 4px 0 0 rgba(101, 113, 255, 0.25); color: #343434; border: 1px solid #B4B2B2; border-radius: 10px; background: rgba(231, 231, 231, 0.75);">${feedback.type}</p>
											</div>
											<div class="mb-3 col-12 text-center d-flex justify-content-center align-items-center">
												<p style="width: 260px; padding-inline: 20px !important; padding: 10px !important; font-size: 16px; box-shadow: 4px 4px 0 0 rgba(101, 113, 255, .25) !important; color: #343434; border: 1px solid #B4B2B2; border-radius: 10px; background: rgba(231, 231, 231, 0.75);">${feedback.feedback}</p>
											</div>
										</div>
									</div>
									`;
							...
							$('#allFeedBackDisplay').html(feedback_html);
						}
					},
		...
		$(document).on('click', '.dashboard-feedback-btn', function() {
			$(this).removeClass('show');
			$('.dashboard-feedback-container').addClass('show')
			$('.dashboard-feedback-items').addClass('show')
		});

		$(document).on('click', '.dashboard-feedback-container-btn', function() {
			$('.dashboard-feedback-btn').addClass('show');
			$('.dashboard-feedback-container').removeClass('show')
			$('.dashboard-feedback-items').removeClass('show')
			$('.dashboard-feedback-info').each(function() {
		...
		$(document).on('click', '.open-feedback', function() {
			...
			$('.dashboard-feedback-info').each(function() {
				if ($(this).attr('id') != `feedback-info-${feed}`) {
					$(this).removeClass('show');
				}
			});
			if($(`#feedIndicator_${feed}`).length > 0){
				$.ajax({
					url: `{{ route('observer.setreviewedfeed') }}`,
					type: 'POST',
					data: {
						feed: feed
					},
			...
			$('.dashboard-feedback-items').removeClass('show');
			$(`#feedback-info-${feed}`).addClass('show');
		});

		$(document).on('click', '.delete-feedback', function() {
			var feed = $(this).data('feed');
			$.ajax({
				url: `{{ route('observer.removefeed') }}`,
				type: 'POST',
				noLoading: true,
				data: {
					feed: feed
				...
				success: function (response) {
					if(response.status === 'success'){
						Toast.fire({ icon: 'success', title: `Successfully remove feedback` });
						$(`#feed-div-${feed}`).remove();
					} else if(response.status === 'error'){
						Toast.fire({
							icon: 'error',
							title: 'Error',
		...
		$(document).on('click', '.dashboard-feedback-info-btn', function() {
			$('.dashboard-feedback-info').each(function() {
				$(this).removeClass('show');
			});
			$('.dashboard-feedback-items').addClass('show');
		...
		$(document).on('keyup', '#searchAllFeedback', function() {
			let searchText = $(this).val().toLowerCase();
			$('.dashboard-feedback-content').each(function () {
		...
		function getAllNotes(){
			const currentUrl = window.location.href;
			const allowedUrl = 'https://tribo.uno/manager/personal_table';
			if(currentUrl !== allowedUrl && currentUrl !== profileUrl){
				$.ajax({
					url: `{{ route('observer.getallnotes') }}`,
					type: 'GET',
					dataType: 'json',
					success: function(response) {
						if(response.status === 'exist' && response.notes.length > 0){
							var notes_html = `
							<button class="btn dashboard-notes-btn show"><i class="mdi mdi-clipboard icon-wiggle" style="font-size: 19px;"></i></button>
							<div class="dashboard-notes-container">
								<div class="input-group dashboard-notes-search">
									<input type="text" class="form-control" id="searchAllNotes" placeholder="Search here...">
								</div>
								<button class="btn dashboard-notes-container-btn"><i class="mdi mdi-close icon-wiggle" style="font-size: 10px;"></i></button>
								<div class="dashboard-notes-items">
							`;

							if(response.notes.length > 0){
								response.notes.forEach(note => {
									notes_html += `
									<div class="dashboard-notes-content" id="notes-div-${note.id}" data-name="${note.title}">
										<div class="dashboard-notes-header text-center">
											<p>Title: <b class="text-primary">${note.title}</b></p>
											${note.description !== null ? `<span class="text-muted" style="font-size: 12px">Description: ${note.description} </span>` :''}
										</div>
										<div class="dashboard-notes-body">
											<p class="text-dark"><i data-feather="maximize" class="icon-lg icon-wiggle open-notes" data-note="${note.id}" type="button"></i></p>
											<p class="text-dark ms-2"><i data-feather="trash" class="icon-lg icon-wiggle delete-notes" type="button" data-note="${note.id}"></i></p>
										</div>
									</div>
									`;
								});
							...
							if(response.notes.length > 0){
								response.notes.forEach(note => {
									notes_html += `
									<div class="dashboard-notes-info" id="notes-info-${note.id}">
										<button class="btn dashboard-notes-info-btn"><i class="mdi mdi-subdirectory-arrow-left icon-wiggle" style="font-size: 15px;"></i></button>
										<form id="notesAllForm_${note.id}">
											<div class="row p-0 py-2">
												...
												<div class="mb-2 px-5">
													...
													<textarea type="text" rows="1" name="description" placeholder="Enter Description" class="form-control convo-info-text">${note.description != null ? note.description : ''}</textarea>
												</div>
												<div class="mb-2">
													<label for="notesNotes" class="ps-4">Notes:</label>
													<textarea type="text" rows="6" name="notes" placeholder="Enter Notes" class="form-control convo-info-text m-0 p-2 rounded-0  border-start-0 border-end-0">${note.notes != null? note.notes : ''}</textarea>
												</div>
												<div class="col-12 px-5">
													<button type="button" class="btn convo-info-btn float-end save-notes" type="button" data-note="${note.id}"><i data-feather="check" class="text-muted icon-wiggle me-2"></i> Save</button>
												</div>
											</div>
										</form>
									</div>
							...
							$('#allNotesDisplay').html(notes_html);
							initializeNoteDrag();
						}
					},
					error: function(xhr, error, status){
		...
		function initializeNoteDrag() {
			let maxZIndex = 100;
			$('.dashboard-notes-container').each(function() {
				...
				header.off('mousedown').on('mousedown', function(e) {
					...
					$('.dashboard-notes-container').not(element).each(function() {
				...
				function dragMouseDown(e) {
					...
					const currentTop = parseInt(element.css('top')) || 0;
					const currentRight = parseInt(element.css('right')) || 0;

					element.data('start-top', currentTop);
					element.data('start-right', currentRight);
		...
		$(document).on('click', '.open-notes', function() {
			...
			$('.dashboard-notes-info').each(function() {
				if ($(this).attr('id') != `notes-info-${note}`) {
					$(this).removeClass('show');
				}
			});

			$('.dashboard-notes-items').removeClass('show');
			$(`#notes-info-${note}`).addClass('show');
		});

		$(document).on('click', '.delete-notes', function() {
			var note = $(this).data('note');
			$.ajax({
				url: '{{route("observer.personal_table.notes_remove")}}',
				type: 'POST',
				noLoading: true,
				data: {
					notes: note,
		...
		$(document).on('click', '.dashboard-notes-btn', function() {
			$(this).removeClass('show');
			$('.dashboard-notes-container').addClass('show')
			$('.dashboard-notes-items').addClass('show')
		});

		$(document).on('click', '.dashboard-notes-container-btn', function() {
			$('.dashboard-notes-btn').addClass('show');
			$('.dashboard-notes-container').removeClass('show')
			$('.dashboard-notes-items').removeClass('show')
			$('.dashboard-notes-info').each(function() {
				if($(this).hasClass('show')){
					$(this).removeClass('show');
		...
		$(document).on('click', '.dashboard-notes-info-btn', function() {
			$('.dashboard-notes-info').each(function() {
				$(this).removeClass('show');
			});
			$('.dashboard-notes-items').addClass('show');
		...
		$(document).on('click', '.save-notes', function() {
			var note = $(this).data('note');
			var form = $(`#notesAllForm_${note}`).serialize();
			$.ajax({
				url: '{{route("observer.personal_table.edit_notes")}}',
				type: 'POST',
				noLoading: true,
				data: form += '&gets=1',
				headers: { 'X-CSRF-TOKEN': token },
				success: function(response) {
					if (response.status == 'success') {
						...
						if(response.note){
							...
							$(`#notes-div-${notes.id}`).html(`
								<div class="dashboard-notes-header text-center">
									<p>Title: <b class="text-primary">${notes.title}</b></p>
									${notes.description !== null ? `<span class="text-muted" style="font-size: 12px">Description: ${notes.description} </span>` :''}
								</div>
								<div class="dashboard-notes-body">
									<p class="text-dark"><i data-feather="maximize" class="icon-lg icon-wiggle open-notes" data-note="${notes.id}" type="button"></i></p>
									<p class="text-dark ms-2"><i data-feather="trash" class="icon-lg icon-wiggle delete-notes" type="button" data-note="${notes.id}"></i></p>
								</div>
							`);
							feather.replace();
						}
		...
		$(document).on('keyup', '#searchAllNotes', function() {
			let searchText = $(this).val().toLowerCase();
			$('.dashboard-notes-content').each(function () {
				let chatName = $(this).attr('data-name').toLowerCase();
		...
		const chatallowedUrl = 'https://tribo.uno/manager/chat';
		...
		if(chatcurrentUrl != chatallowedUrl && chatcurrentUrl !== profileUrl){
			...
			function getAllChats(){
				$.ajax({
					url: `{{ route('observer.getallchats') }}`,
					type: 'GET',
					success: function(response) {
			...
			$(document).on('click', '#sendToUserMessage', function () {
				...
				$.ajax({
					url: `{{ route('observer.chat.sendmessage') }}`,
					type: 'POST',
					noLoading: true,
					data: formData,
			...
			function checkMedia(messageId, isMe) {
				$.ajax({
					url: "{{ route('observer.chat.checkmessageattachment') }}",
					type: "GET",
			...
			function buildTask(messageId, taskId, isMe, user_id){
				$.ajax({
					url: "{{ route('observer.chat.gettaskinfo') }}",
					type: "GET",
					data: { task: taskId },
					dataType: 'json',
					success: function(response) {
						if (response.status === 'error') {
							...
							if (response.task) {
								...
								let viewUrl = "{{ route('observer.lvtasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', task.id);
								let editUrl = "{{ route('observer.etasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', task.id);
								const task_html = $(
									`
			...
			$(document).on('click', '#cancelEmergency', function() {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						window.location.href = `/manager/etasks/${task}`;
					} else {
						return;
					}
				});
