                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-12 d-grid mb-2">
                                    <button type="button" class="btn convo-info-btn btn-sm" data-bs-toggle="collapse" data-bs-target="#collapseDepartment" aria-expanded="false" aria-controls="collapseDepartment">
                                        <i data-feather="server" class="cursor-pointer icon-sm icon-wiggle"></i> Department's Event
                                    </button>
                                </div>
                                <div class="col-12 collapse"  id="collapseDepartment">
                                    <div id='department-events-external' class='external-events'>
                                        <h6 class="mb-2 text-muted">Department Events</h6>
                                        <div data-type="department" class='fc-event fc-h-event fc-daygrid-event fc-daygrid-block-event'>
                                            <div class='fc-event-main'>Department Meeting</div>
                                        </div>
                                        <div data-type="department" class='fc-event fc-h-event fc-daygrid-event fc-daygrid-block-event'>
                                            <div class='fc-event-main'>New Project</div>
                                        </div>
                                        <div data-type="department" class='fc-event fc-h-event fc-daygrid-event fc-daygrid-block-event'>
                                            <div class='fc-event-main'>Department Announcement</div>
                                        </div>
                                        <div data-type="department" class='fc-event fc-h-event fc-daygrid-event fc-daygrid-block-event'>
                                            <div class='fc-event-main'>Team Building Activities</div>
                                        </div>
                                        <div data-type="department" class='fc-event fc-h-event fc-daygrid-event fc-daygrid-block-event'>
                ...
                <div id="modalBody2" class="modal-body">
                    <form id="eventForm">
                        ...
                        <select class="form-select convo-info-text mb-2" id="eventType" aria-label="Default select example">
                            ...
                            <option value="department">Department Announcment</option>
                        </select>
...
$(document).ready(function() {
    ...
    if(hasDepartment == 'yes'){
        var containerDepartment = document.getElementById('department-events-external');
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: function(fetchInfo, successCallback, failureCallback) {
            ...
            if(hasDepartment == 'yes'){
                $.ajax({
                    url: '{{route("observer.calendar.viewtaskdate")}}',
                    type: 'GET',
                    dataType: 'json',
            ...
            $.ajax({
                url: '{{route("observer.calendar.viewprivateeventdate")}}',
                type: 'GET',
                dataType: 'json',
                success: function(response) {
            ...
            if(hasDepartment == 'yes'){
                ...
                $.ajax({
                    url: '{{route("observer.calendar.viewdepartmenteventdate")}}',
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
                        if (response.status !== 'nothing') {
            ...
            $.ajax({
                url: '{{route("observer.calendar.viewannouncementeventdate")}}',
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    if (response.status !== 'nothing') {
            ...
            Swal.fire({
                ...
                didOpen: () => {
                    ...
                    $('#eventFormDrag').submit(function(e) {
                        ...
                        $.ajax({
                            url: '{{route("observer.calendar.saveevent")}}',
                            type: 'POST',
        ...
        eventClick: function(info) {
            ...
            if(event.extendedProps.source == 'task-calendar'){
                ...
                var observerTaskRoute = @json(route('observer.lvtasks', ['task' => '__TASK_ID__']));
            ...
            if(event.extendedProps.source == 'private' || event.extendedProps.source == 'department' || event.extendedProps.source == 'announcement'){
                ...
                $('#fullCalModal').find('.modal-footer').html(`
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                ${private.source !== 'announcement' && private.user_id == @json(Auth::id()) ? `
    ...
    $(document).on('click', '.removeEvent', function() {
        ...
        $.ajax({
            url: '{{route("observer.calendar.removeevent")}}',
            type: 'POST',
            data: {
                event: event
