                            ...
                            @if(!empty($temp))
                                ...
                                <tr>
                                    ...
                                    <td class="action-buttons">
                                        ...
                                        <button type="button" class="btn btn-primary mx-1 btn-hover" id="btnDistribute" data-temp="{{$row->id}}" @if($row->pages == 0) disabled @endif><i data-feather="folder-plus" class="icon-sm icon-wiggle"></i></button>
                                                    ...
                                                    @foreach($distReqView as $row)
                                                        <tr id="distReqRow_{{$row->id}}">
                                                            ...
                                                            <td class="action-buttons">
                                                                ...
                                                                <a class="btn btn-primary mx-1 btn-hover" href="{{ route('head.lvtasks', ['task' => $row->task_id]) }}"><i data-feather="eye" class="icon-sm icon-wiggle"></i></a>
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                @endif
                                                ...
                                                @foreach($dist as $row)
                                                    <tr id="distRow_{{$row->id}}">
                                                        ...
                                                        <td class="action-buttons">
                                                            ...
                                                            <a class="btn btn-primary mx-1 btn-hover" href="{{ route('head.lvtasks', ['task' => $row->id]) }}"><i data-feather="eye" class="icon-sm icon-wiggle"></i></a>
                                                            <a class="btn btn-primary mx-1 btn-hover" href="{{ route('head.ptasks', ['task' => $row->id]) }}"><i data-feather="printer" class="icon-sm icon-wiggle"></i></a>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="title" class="form-control" id="title" aria-describedby="title" onkeydown="return event.key !== 'Enter';">
                                </div>
                            </div>
                        </div>
                    </div>
...
//region Code
        ...
        $('#page').load(location.href + " #page > *", function() {
            ...
            if ($('#taskDonut').length) {
                ...
                Morris.Donut({
                    ...
                    data: [
                        { label: "To Assign", value: {{ $temp->count()}} } ,
                        { label: "On-Going", value: {{ $ongoing->count()}} } ,
                        { label: "To Check", value: {{ $tocheck->count()}} } ,
                        { label: "Complete", value: {{ $complete->count()}} } ,
                        { label: "Distributed", value: {{ $dist->count()}} } ,
                        { label: "Linked", value: {{ $linked->count()}} }
                    ],
    ...
    function reloadOngoingDiv() {
        $.ajax({
            url: "{{ route('reloadhe.ongoing.div') }}",
            type: "POST",
            noLoading: true,
            data: {
    ...
    function reloadToCheckDiv() {
        $.ajax({
            url: "{{ route('reloadhe.tocheck.div') }}",
            type: "POST",
            noLoading: true,
            data: {
                _token: "{{ csrf_token() }}", // CSRF token for Laravel
                ...
                if (response.status === 'initial_load') {
                    ...
                    if (response.tasks && response.tasks.length > 0) {
                        response.tasks.forEach(task => {
                            ...
                            let taskId = task.id;
                            let viewUrl = "{{ route('head.lvtasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', taskId);
    ...
    function reloadComplete() {
        $.ajax({
            url: "{{ route('reloadhe.complete.div') }}",
            type: "POST",
            noLoading: true,
            data: {
                _token: "{{ csrf_token() }}", // CSRF token for Laravel
                ...
                if (response.status === 'initial_load') {
                    ...
                    if (response.tasks && response.tasks.length > 0) {
                        response.tasks.forEach(task => {
                            ...
                            let taskId = task.id;
                            let viewUrl = "{{ route('head.lvtasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', taskId);
                            let printUrl = "{{ route('head.ptasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', taskId);
                            const rowNode = table.row.add([
                                task.title,
                                `<span class="badge ${task.status === 'Overdue' ? 'bg-danger' : 'bg-primary'}">${task.status}</span>`,
                                task.assigned_to,
    ...
    function reloadDistribute() {
        $.ajax({
            url: "{{ route('reloadhe.distribute.div') }}",
            type: "POST",
            noLoading: true,
            data: {
                _token: "{{ csrf_token() }}", // CSRF token for Laravel
    ...
    function reloadDistributeReq() {
        $.ajax({
            url: "{{ route('reloadhe.distributereq.div') }}",
            type: "POST",
            noLoading: true,
            data: {
                _token: "{{ csrf_token() }}", // CSRF token for Laravel
    ...
    function reloadOvertimeRequest() {
        $.ajax({
            url: "{{ route('reloadhe.overtimereq.div') }}",
            type: "POST",
            noLoading: true,
            data: {
                _token: "{{ csrf_token() }}", // CSRF token for Laravel
    ...
    $(document).on('click', '#approveTask', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.approvetask") }}',
                    method: 'POST',
                    data: {
                        task: task
                    },
    ...
    $(document).on('click', '#declineTask', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '{{ route("head.tasks.declinetask") }}',
                    method: 'POST',
                    data: {
                        task: task,
    ...
    $(document).on('click', '#approveTaskDistribution', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.adisttask") }}',
                    method: 'POST',
                    data: {
                        dist: dist
                    },
    ...
    $(document).on('click', '#declineTaskDistribution', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.ddisttask") }}',
                    method: 'POST',
                    data: {
                        dist: dist
                    },
    ...
    $(document).on('click', '#approveAllTaskDistribution', function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.aadisttask") }}',
                    method: 'POST',
                    data: {
                        dist: 0
                    },
    ...
    $(document).on('click', '#declineAllTaskDistribution', function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.dadisttask") }}',
                    method: 'POST',
                    data: {
                        dist: 0
                    },
            ...
            success: function(response) {
                ...
                $(document).on('change', '#templates', function(){
                    ...
                    if(assign === 0){
                        ...
                        $.ajax({
                            url: '{{ route("head.tasks.checklinktemp") }}',
                            method: 'GET',
                            data: {
                                task: taskValue
                            },
                                ...
                                if(response.status === 'exist'){
                                    Swal.fire({
                                        ...
                                        if (result.isConfirmed) {
                                            ...
                                            $.ajax({
                                                url: '{{ route("head.tasks.removelinktemp") }}',
                                                method: 'POST',
                                                data: {
                                                    task: taskValue,
                                                },
    ...
    $(document).on('click', '.submitMemberLinkTempoList', function() {
        ...
        $.ajax({
            url: '{{ route("head.tasks.addmemberlinktemp") }}',
            method: 'POST',
            noLoading: true,
            data: {
                task: task,
    ...
    $(document).on('click', '.removeMemberLinkTempoList', function() {
        ...
        $.ajax({
            url: '{{ route("head.tasks.removememberlinktemp") }}',
            method: 'POST',
            noLoading: true,
            data: {
                task: task,
    ...
    $(document).on('click', '.submitSoloLink', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.linkandassigntask") }}',
                    method: 'POST',
                    data: form,
                    dataType: 'json',
                    headers: {
    ...
    $(document).on('click', '.submitGroupLink', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.grouplinkandassigntask") }}',
                    method: 'POST',
                    data: form,
                    dataType: 'json',
                    headers: {
    ...
    $(document).on('click', '.linkToOngoingTask', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.linkandassignongoingtask") }}',
                    method: 'POST',
                    data: {
                        task: task,
                        link: link
...
//region Link Task
    ...
    $(document).on('click', '#viewLinkedTask', function() {
        var task = $(this).data('task');
        window.location.href = `/head/lvtasks/${task}`;
    });
...
//region User Statistic
    ...
    $(document).on('click', '#viewUserStatusStatistic', function() {
        ...
        $.ajax({
            url: '{{ route("head.tasks.viewstatistic") }}',
            method: 'GET',
            data: {
                task: task
            },
    ...
    $(document).on('click', '#emergencyButton', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.setemergencyuserstatus") }}',
                    method: 'POST',
                    data: {
                        task: task
                    },
...
//region Working Time
    ...
    $(document).on('click', '.acceptOvertimeBtn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.acceptovertimerequest") }}',
                    method: 'POST',
                    data: {
                        task: task
                    },
    ...
    $(document).on('click', '.declineOvertimeBtn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.declineovertimerequest") }}',
                    method: 'POST',
                    data: {
                        task: task
                    },
...
//region Chat Task Member
    ...
    $('#messageContactForm').on('submit', function (e) {
        ...
        $.ajax({
            url: `{{ route('head.chat.sendtaskcontactmessage') }}`,
            type: 'POST',
            noLoading: true,
            data: formData,
...
//region Archive Task
    ...
    $(document).on('click', '.archiveDistributeTask', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.archivedistributedtask") }}',
                    method: 'POST',
                    data: {
                        task: task
                    },
    ...
    $(document).on('click', '.archiveCompletedTask', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.archivecompletedtask") }}',
                    method: 'POST',
                    data: {
                        task: task
                    },
    ...
    $(document).on('click', '#retrieveTask', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.retrievetask") }}',
                    method: 'POST',
                    data: {
                        task: task
                    },
    ...
    $(document).on('click', '.deleteTask', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.deletetask") }}',
                    method: 'POST',
                    data: {
                        task: task
                    },
    ...
    $(document).on('click', '#retrieveTemp', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.retrievetemp") }}',
                    method: 'POST',
                    data: {
                        temp: temp
                    },
    ...
    $(document).on('click', '.deleteTemp', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("head.tasks.deletetemp") }}',
                    method: 'POST',
                    data: {
                        temp: temp
                    },
