                ...
                <div class="dropdown-menu p-0" aria-labelledby="profileDropdown">
                    ...
                    <ul class="list-unstyled p-1">
                        <li class="dropdown-item py-2">
                            <a href="{{ route('head.profile') }}" class="text-body ms-0">
                                <i class="me-2 icon-md icon-wiggle" data-feather="user"></i>
                                <span>Profile</span>
                            </a>
                        </li>
                        <li class="dropdown-item py-2">
                            <a href="{{ route('head.logout') }}" class="text-body ms-0">
...
$(document).ready(function () {
    ...
    function getAllNotif(){
        $.ajax({
            url: `{{ route('head.getallnotification') }}`,
            type: 'GET',
            dataType: 'json',
    ...
    $(document).on('click', '.markAsReadNotification', function() {
        $.ajax({
            url: `{{ route('head.markasreadednotification') }}`,
            type: 'POST',
            noLoading: true,
            data: {
                read: 1
    ...
    $(document).on('click', '.clearMyNotification', function() {
        $.ajax({
            url: `{{ route('head.clearnotification') }}`,
            type: 'POST',
            noLoading: true,
            data: {
                read: 1
