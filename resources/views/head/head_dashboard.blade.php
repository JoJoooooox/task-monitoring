...
<body>
	...
	<div class="main-wrapper">
        ...
        @include('head.body.sidebar')

        <div class="page-wrapper">

            <!-- partial:partials/_navbar.html -->
            @include('head.body.header')
            <!-- partial -->

            @yield('head')

            <!-- partial:partials/_footer.html -->
            @include('head.body.footer')
            <!-- partial -->
	...
	<script src="{{ asset('assets/custom-js/head.js') }}"></script>
	<script src="{{ asset('assets/vendors/chartjs/Chart.min.js') }}"></script>
	...
	$(document).ready(function () {
        ...
        function forceLogout() {
            window.location.href = "/head/logout"; // Update with your logout route
        }
		...
		function checkUrlUser() {
			...
			const allowedUrl = 'https://tribo.uno/head/chat';

			if (currentUrl !== allowedUrl) {
				$.ajax({
					url: "{{ route('head.chat.unsetchatishere') }}",
					type: "POST",
					noLoading: true,
					headers: {
				...
				$.ajax({
					url: "{{ route('head.chat.setchatishere') }}",
					type: "POST",
					noLoading: true,
					headers: {
                        'X-CSRF-TOKEN': tokenDashboard
		...
		const profileUrl = 'https://tribo.uno/head/profile';

		const chatcurrentUrl = window.location.href;
		const chatallowedUrl = 'https://tribo.uno/head/chat';
