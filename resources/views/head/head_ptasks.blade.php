...
$(document).ready(function(){
    ...
    $("#printButton").click(function () {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                printWindow.document.write('<link rel="stylesheet" href="/assets/custom-css/head.css">');
                ...
                setTimeout(() => {
                    printWindow.print();
                    printWindow.close();
                    window.location.href = "{{ route('head.tasks')}}";
                }, 500);
            }
        });
    ...
    function getRadioField(id) {
        return new Promise(function (resolve, reject) {
            $.ajax({
                url: '{{ route("head.tasks.printgetradio") }}',
                method: 'GET',
                data: { id: id },
                dataType: 'json',
                success: function(response) {
    ...
    function getDownField(id) {
        return new Promise(function (resolve, reject) {
            $.ajax({
                url: '{{ route("head.tasks.printgetdown") }}',
                method: 'GET',
                data: { id: id },
                dataType: 'json',
                success: function(response) {
    ...
    function reloadOngoingDiv() {
        var id = $('#panelTask').data('task');
        $.ajax({
            url: "{{ route('head.tasks.liveprintreloading') }}",
            type: "POST",
            noLoading: true,
            data: {
                _token: "{{ csrf_token() }}",
