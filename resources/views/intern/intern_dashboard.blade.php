...
<body>
	...
	$(document).ready(function () {
        ...
        function forceLogout() {
            window.location.href = "/intern/logout"; // Update with your logout route
        }
		...
		function checkUrlUser() {
			...
			const allowedUrl = 'https://tribo.uno/intern/chat';

			if (currentUrl !== allowedUrl) {
				$.ajax({
					url: "{{ route('intern.chat.unsetchatishere') }}",
					type: "POST",
					noLoading: true,
					headers: {
				...
				$.ajax({
					url: "{{ route('intern.chat.setchatishere') }}",
					type: "POST",
					noLoading: true,
					headers: {
                        'X-CSRF-TOKEN': tokenDashboard
		...
		const profileUrl = 'https://tribo.uno/intern/profile';
		...
		function getAllFeedback(){
			...
			if(currentUrl !== profileUrl){
				$.ajax({
					url: `{{ route('intern.getallfeedback') }}`,
					type: 'GET',
					dataType: 'json',
					success: function(response) {
		...
		$(document).on('click', '.open-feedback', function() {
			...
			if($(`#feedIndicator_${feed}`).length > 0){
				$.ajax({
					url: `{{ route('intern.setreviewedfeed') }}`,
					type: 'POST',
					data: {
						feed: feed
					},
		...
		function getAllNotes(){
			const currentUrl = window.location.href;
			const allowedUrl = 'https://tribo.uno/intern/personal_table';
			if(currentUrl !== allowedUrl && currentUrl !== profileUrl){
				$.ajax({
					url: `{{ route('intern.getallnotes') }}`,
					type: 'GET',
					dataType: 'json',
					success: function(response) {
		...
		const chatallowedUrl = 'https://tribo.uno/intern/chat';
		...
		if(chatcurrentUrl != chatallowedUrl && chatcurrentUrl !== profileUrl){
			...
			$(document).on('click', '#sendToUserMessage', function () {
				...
				$.ajax({
					url: `{{ route('intern.chat.sendmessage') }}`,
					type: 'POST',
					data: formData,
			...
			function checkMedia(messageId, isMe) {
				$.ajax({
					url: "{{ route('intern.chat.checkmessageattachment') }}",
					type: "GET",
			...
			function buildTask(messageId, taskId, isMe, user_id){
				$.ajax({
					url: "{{ route('intern.chat.gettaskinfo') }}",
					type: "GET",
					data: { task: taskId },
					dataType: 'json',
					success: function(response) {
						if (response.status === 'error') {
							...
							if (response.task) {
								...
								let viewUrl = "{{ route('intern.lvtasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', task.id);
								let editUrl = "{{ route('intern.etasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', task.id);
								const task_html = $(
									`
			...
			$(document).on('click', '#cancelEmergency', function() {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						window.location.href = `/intern/etasks/${task}`;
					} else {
						return;
					}
				});
			...
			function reloadChatMessage() {
				if($('.dashboard-chat-box.show').length > 0){
					...
					$.ajax({
						url: "{{ route('reloadint.chat.message') }}",
						type: "POST",
						noLoading: true,
						data: {
							_token: "{{ csrf_token() }}",
