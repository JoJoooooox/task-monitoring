                                                    ...
                                                    @foreach($tasks as $row)
                                                        <tr id="holdingRow_{{$row->id}}" class="{{$row->status === 'Overdue' ? 'flicker' : ''}}">
                                                            ...
                                                            <td class="action-buttons">
                                                                <a class="btn btn-primary mx-1 btn-hover" href="{{ route('intern.lvtasks', ['task' => $row->id]) }}"><i data-feather="eye" class="icon-sm icon-wiggle"></i></a>
                                                                @if(($row->user_status !== 'Emergency' && $row->user_status !== 'Sleep' && $row->user_status !== 'Request Overtime'))
                                                                <a class="btn btn-primary mx-1 btn-hover" href="{{ route('intern.etasks', ['task' => $row->id]) }}"><i data-feather="edit-2" class="icon-sm icon-wiggle"></i></a>
                                        ...
                                        @if(!empty($complete))
                                            @foreach($complete as $row)
                                                <tr id="completeRow_{{$row->id}}" class="{{$row->status === 'Overdue' ? 'flicker' : ''}}">
                                                    ...
                                                    <td class="action-buttons">
                                                        <a class="btn btn-primary mx-1 btn-hover" href="{{ route('intern.lvtasks', ['task' => $row->id]) }}"><i data-feather="eye" class="icon-sm icon-wiggle"></i></a>
                                                        <a class="btn btn-primary mx-1 btn-hover" href="{{ route('intern.ptasks', ['task' => $row->id]) }}"><i data-feather="printer" class="icon-sm icon-wiggle"></i></a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        @endif
...
$(document).ready(function(){
    ...
    function reloadOngoingDiv() {
        $.ajax({
            url: "{{ route('reloadint.ongoing.div') }}",
            type: "POST",
            noLoading: true,
            data: {
    ...
    function reloadToCheckDiv() {
        $.ajax({
            url: "{{ route('reloadint.tocheck.div') }}",
            type: "POST",
            noLoading: true,
            data: {
                _token: "{{ csrf_token() }}", // CSRF token for Laravel
                ...
                if (response.status === 'initial_load') {
                    ...
                    if (response.tasks && response.tasks.length > 0) {
                        response.tasks.forEach(task => {
                            ...
                            let taskId = task.id;
                            let viewUrl = "{{ route('intern.lvtasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', taskId);
    ...
    function reloadComplete() {
        $.ajax({
            url: "{{ route('reloadint.complete.div') }}",
            type: "POST",
            noLoading: true,
            data: {
                _token: "{{ csrf_token() }}", // CSRF token for Laravel
                ...
                if (response.status === 'initial_load') {
                    ...
                    if (response.tasks && response.tasks.length > 0) {
                        response.tasks.forEach(task => {
                            ...
                            let taskId = task.id;
                            let viewUrl = "{{ route('intern.lvtasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', taskId);
                            let printUrl = "{{ route('intern.ptasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', taskId);
                            const rowNode = table.row.add([
                                task.title,
                                `<span class="badge ${task.status === 'Overdue' ? 'bg-danger' : 'bg-primary'}">${task.status}</span>`,
                                task.assigned_to,
    ...
    function addRowToTable(task){
        ...
        let viewUrl = "{{ route('intern.lvtasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', taskId);
        let editUrl = "{{ route('intern.etasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', taskId);
    ...
    $(document).on('click', '#viewLinkedTask', function() {
        ...
        window.location.href = `/intern/lvtasks/${task}`;
    });
