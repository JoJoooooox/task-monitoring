                                        ...
                                        @foreach($tag as $row)
                                            ...
                                            <li class="nav-item d-grid col-12 p-1" data-task-id="{{ $row->id }}" data-name="{{$row->title}}">
                                                <div class="d-flex align-items-center justify-content-between">
                                                    <a class="nav-link d-flex align-items-center flex-grow-1" href="{{ route('employee.etasks', ['task' => $row->id]) }}">
                                                    ...
                                                    <div class="d-flex align-items-center gap-2 ms-3">
                                                        <a href="{{ route('employee.lvtasks', ['task' => $row->id]) }}" class="btn btn-sm btn-light">
                                        ...
                                        <div class="personal-options ms-auto">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu">
                                                    @if($row->user_status !== 'Emergency' && $row->user_status !== 'Sleep' && $row->user_status !== 'Request Overtime')
                                                    <a class="dropdown-item d-flex align-items-center" href="{{ route('employee.etasks', ['task' => $row->id]) }}">
                                                    ...
                                                    <a class="dropdown-item d-flex align-items-center" href="{{ route('employee.lvtasks', ['task' => $row->id]) }}">
                                            ...
                                            <div class="personal-options ms-auto">
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        ...
                                                        <a class="dropdown-item d-flex align-items-center" href="{{ route('employee.etasks', ['task' => $relatedTasks[$task_id]->id]) }}">
                                                            <i data-feather="edit-2" class="icon-sm me-2"></i> Edit Task
                                                        </a>
                                                        ...
                                                        <a class="dropdown-item d-flex align-items-center" href="{{ route('employee.lvtasks', ['task' => $relatedTasks[$task_id]->id]) }}">
                                                            <i data-feather="eye" class="icon-sm me-2"></i> View Task
                                                        </a>
                                                    </div>
                                                </div>
    ...
    setTimeout(() => {
        if ($("#shortcut-list").length) {
            ...
            function updateSortOrder() {
                ...
                $.ajax({
                    url: '{{ route("employee.personal_table.update_sort") }}',
                    method: 'POST',
                    data: {
                        type: 'tag',
...
$(document).ready(function() {
    ...
    $(document).on('click', '.sort-option', function(e) {
        ...
        $.ajax({
            url: "{{ route('employee.personal_table_sort') }}",
            type: 'GET',
            data: { sort: sortBy },
            success: function(response) {
                if(response.status == 'success'){
    ...
    $(document).on('click', '.markAsPersonal', function() {
        ...
        $.ajax({
            url: '{{route("employee.personal_table.mark_task")}}',
            type: 'POST',
            data: $.param(formData),
    ...
    $(document).on('click', '.favoritesTask', function() {
        ...
        $.ajax({
            url: '{{route("employee.personal_table.task_favorites")}}',
            type: 'POST',
    ...
    $(document).on('click', '.importantTask', function() {
        ...
        $.ajax({
            url: '{{route("employee.personal_table.task_important")}}',
            type: 'POST',
    ...
    $(document).on('click', '.shortcutTask', function() {
        ...
        $.ajax({
            url: '{{route("employee.personal_table.task_tag")}}',
            type: 'POST',
    ...
    $(document).on('click', '.taskNotesSubmit', function() {
        ...
        $.ajax({
            url: '{{route("employee.personal_table.task_notes")}}',
            type: 'POST',
            data: form,
            headers: { 'X-CSRF-TOKEN': token },
            success: function(response) {
    ...
    $(document).on('click', '.remove-notes', function() {
        ...
        var task = $(this).data('task');
        $.ajax({
            url: '{{route("employee.personal_table.task_notes_remove")}}',
            type: 'POST',
    ...
    $(document).on('click', '.createPrivateNotes', function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '{{ route("employee.personal_table.create_note") }}',
                    method: 'POST',
                    data: {
