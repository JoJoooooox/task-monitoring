...
<script src="https://meet.jit.si/external_api.js"></script>
	...
	$(document).ready(function () {
        ...
        function forceLogout() {
            window.location.href = "/employee/logout"; // Update with your logout route
        }
		...
		function checkUrlUser() {
			...
			const allowedUrl = 'https://tribo.uno/employee/chat';

			if (currentUrl !== allowedUrl) {
				$.ajax({
					url: "{{ route('employee.chat.unsetchatishere') }}",
					type: "POST",
					noLoading: true,
					headers: {
				...
				$.ajax({
					url: "{{ route('employee.chat.setchatishere') }}",
					type: "POST",
					noLoading: true,
					headers: {
                        'X-CSRF-TOKEN': tokenDashboard
		...
		const profileUrl = 'https://tribo.uno/employee/profile';
		...
		function getAllFeedback(){
			...
			if(currentUrl !== profileUrl){
				$.ajax({
					url: `{{ route('employee.getallfeedback') }}`,
					type: 'GET',
					dataType: 'json',
					success: function(response) {
		...
		$(document).on('click', '.open-feedback', function() {
			...
			if($(`#feedIndicator_${feed}`).length > 0){
				$.ajax({
					url: `{{ route('employee.setreviewedfeed') }}`,
					type: 'POST',
					noLoading: true,
					data: {
						feed: feed
		...
		function getAllNotes(){
			const currentUrl = window.location.href;
			const allowedUrl = 'https://tribo.uno/employee/personal_table';
			if(currentUrl !== allowedUrl && currentUrl !== profileUrl){
				$.ajax({
					url: `{{ route('employee.getallnotes') }}`,
					type: 'GET',
					dataType: 'json',
					success: function(response) {
		...
		$(document).on('click', '.delete-notes', function() {
			var note = $(this).data('note');
			$.ajax({
				url: '{{route("employee.personal_table.notes_remove")}}',
				type: 'POST',
				data: {
					notes: note,
					dashboard: 1
		...
		$(document).on('click', '.save-notes', function() {
			...
			$.ajax({
				url: '{{route("employee.personal_table.edit_notes")}}',
				type: 'POST',
				data: form += '&gets=1',
				headers: { 'X-CSRF-TOKEN': token },
				success: function(response) {
		...
		const chatallowedUrl = 'https://tribo.uno/employee/chat';
		...
		if(chatcurrentUrl != chatallowedUrl && chatcurrentUrl !== profileUrl){
			...
			function getAllChats(){
				$.ajax({
					url: `{{ route('employee.getallchats') }}`,
					type: 'GET',
					success: function(response) {
			...
			$(document).on('click', '#sendToUserMessage', function () {
				...
				$.ajax({
					url: `{{ route('employee.chat.sendmessage') }}`,
					type: 'POST',
					data: formData,
					noLoading: true,
			...
			function checkMedia(messageId, isMe) {
				$.ajax({
					url: "{{ route('employee.chat.checkmessageattachment') }}",
					type: "GET",
			...
			function buildTask(messageId, taskId, isMe, user_id){
				$.ajax({
					url: "{{ route('employee.chat.gettaskinfo') }}",
					type: "GET",
					data: { task: taskId },
					dataType: 'json',
					success: function(response) {
						if (response.status === 'error') {
							...
							if (response.task) {
								...
								let viewUrl = "{{ route('employee.lvtasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', task.id);
								let editUrl = "{{ route('employee.etasks', ['task' => '__TASK_ID__']) }}".replace('__TASK_ID__', task.id);
								const task_html = $(
									`
			...
			function reloadChatMessage() {
				if($('.dashboard-chat-box.show').length > 0){
					...
					$.ajax({
						url: "{{ route('reloademp.chat.message') }}",
						type: "POST",
						noLoading: true,
						data: {
							_token: "{{ csrf_token() }}",
