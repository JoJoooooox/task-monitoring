...
$(document).ready(function(){
    ...
    function liveTask(active = null){
        ...
        $.ajax({
            url: '{{ route("admin.tasks.glvtasks") }}',
            method: 'GET',
            data: {
                task: id
            },
    ...
    function getRadioField(id) {
        return new Promise(function (resolve, reject) {
            $.ajax({
                url: '{{ route("admin.tasks.getradio") }}',
                method: 'GET',
                data: { id: id },
                dataType: 'json',
                success: function(response) {
    ...
    function reloadOngoingDiv() {
        var id = $('#panelTask').data('task');
        $.ajax({
            url: "{{ route('admin.tasks.livereloading') }}",
            type: "POST",
            noLoading: true,
            data: {
                _token: "{{ csrf_token() }}",
    ...
    $(document).on('click', '#unlinkTask', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.tasks.unlinktask") }}',
                    method: 'POST',
                    data: {
                        task: task
                    },
